...
export default function AddInvestorForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      payload.append('phone_number', formData.phone ? `+91 ${formData.phone}` : "0000000000");
          ...
          <div className="bg-white rounded-3xl p-8 border border-gray-100 shadow-sm relative overflow-hidden">
            ...
            <div className="grid md:grid-cols-2 gap-x-8 gap-y-6">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus-within:ring-2 focus-within:ring-emerald-500 focus-within:bg-white transition-all">
                  <span className="text-gray-500 font-bold shrink-0">+91</span>
                  <div className="w-px h-5 bg-gray-300 shrink-0"></div>
                  <input required type="tel" name="phone" value={formData.phone} onChange={handleChange} maxLength={10} className="flex-grow bg-transparent outline-none font-semibold text-gray-900 tracking-wider" placeholder="98765 43210" />
                </div>
              </div>
              <div className="md:col-span-2">
                <label className="block text-xs font-black uppercase tracking-widest text-gray-400 mb-2">Company URL / Website</label>
                <input type="url" name="websiteUrl" value={formData.websiteUrl} onChange={handleChange} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:bg-white font-semibold" placeholder="https://www.example.com" />
              </div>
            </div>
          </div>
