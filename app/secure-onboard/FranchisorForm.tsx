...
export default function FranchisorAddFormSecure({ initialEmail, secureToken }: { initialEmail: string, secureToken: string }) {
  ...
  const isStepValid = () => {
    if (step === 1) return !errors.brandName && !errors.category && !errors.productCategory && !errors.description && !errors.contactPersonName && !errors.emailAddress && !errors.contactNumber;
    ...
    if (type === 'checkbox') {
      ...
      if (name === 'contactNumber') {
        ...
        if (val.length > 10 && val.startsWith('91')) {
          val = val.substring(2); // Remove 91 if it was typed/pasted
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      payload.append('contact_number', `+91 ${formData.contactNumber}`);
    ...
    if (registrationId) {
      try {
        ...
        await fetch(`${API_URL}/api/registrations/trigger-onboarding-email/`, {
          ...
          body: JSON.stringify({ id: registrationId, type: 'franchisor' })
        });
      } catch (e) {
    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center p-6 animate-in fade-in zoom-in duration-500">
        <div className="bg-white rounded-[2rem] p-10 shadow-2xl max-w-md w-full text-center border border-gray-100">
          ...
          <p className="text-gray-500 font-medium mb-6">Your profile data has been safely submitted and linked. A confirmation email will be sent once you choose an option below.</p>
            ...
            <div className="animate-in fade-in slide-in-from-right-8 duration-500">
              ...
              <div className="space-y-6">
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-xs font-black uppercase tracking-widest text-gray-400 mb-2">Brand Logo</label>
                ...
                <div className="pt-4 border-t border-gray-100">
                  ...
                  <div className="grid md:grid-cols-2 gap-5">
                    ...
                    <div className="md:col-span-2">
                      <label className="block text-xs font-black uppercase tracking-widest text-gray-400 mb-2">Company URL / Website</label>
                      <input type="url" name="websiteUrl" value={formData.websiteUrl} onChange={handleChange} placeholder="https://www.brand.com" className="w-full px-5 py-4 bg-gray-50 border border-gray-200 rounded-2xl focus:ring-4 focus:ring-gray-200 focus:bg-white transition-all font-semibold outline-none" />
                    </div>
                  </div>
                </div>
              </div>
